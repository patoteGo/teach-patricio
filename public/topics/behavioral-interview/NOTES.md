...
- Session 2 (fan-out deep dive): user confirmed downstream fan-out was REAL (event push, not pull). Rewrote the "one big table?" model answer in Lesson 5 to the two-paths framing; shipped Lesson 6 (`0006-fanout-decoded.html`): SVG diagrams of the trap (shared-table distributed monolith) vs. real design (Path A push fan-out / Path B persisted query table), SNS→SQS-per-consumer pattern with AWS citation, sentence-by-sentence decode, 4-question retrieval check. Confirmed Cencosud detail worth remembering: per-consumer queues → slow consumer isolation.
- Session 3 (monitoring deep dive): expanded the "how did you know it was falling behind?" probe into Lesson 7 (`0007-monitoring-lag-decoded.html`): SVG pipeline with sensor locations, interactive 8-signal explorer (leading vs lagging vs anti-signal, each with "what it hides" + say-it line), live canvas queue simulator (inflow/pod sliders, sale-day spike, kill-a-pod, HPA-on-lag toggle — demonstrates green p95 while lag climbs), 6 interactive tuning knobs (visibility timeout, batch size, long polling, concurrency, maxReceiveCount, autoscaling signal), 30s answer, 5-question check. Linked from Lesson 5's monitoring probe; added lessons 05–07 to nav manifest + README.
